describe("Test Runner E2E", () => {
  ...
  it("should switch between test scenarios when clicking test buttons", async () => {
    ...
    const characterPosition = await page.$eval('.character', (el: HTMLElement) => {
  ...
  it("should generate frames for scrubber navigation", async () => {
    ...
    const maxFrames = await page.$eval('[data-testid="scrubber-range-input"]', (el: HTMLInputElement) => {
      return parseInt(el.max);
    });
